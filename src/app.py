...
#             url="https://dsb1qxkgnj.execute-api.ap-southeast-1.amazonaws.com/dev/chat/multillm",#settings.API_URL,
#             json={"prompt":prompt},
#             headers=headers
#         )
#         print(response.json())
...
def fetch_bedrock_response(prompt):
    try:
        headers = {
            "x-api-key": "koStY9EEHaaLwi2mL7FvW9xO3q9xnoaS9HTjzMup",
            "Content-Type": "application/json"
        }
        response = requests.post(
...
# -------------------------
def chat_with_kb(user_input):
    raw = fetch_bedrock_response(user_input)
    answer = "No response received from backend."
    sql_query = ""

    if raw and raw.status_code == 200:
        raw_json = raw.json()
        answer = raw_json.get("summary", "No summary available.")
        sql_query = raw_json.get("sql_query", "")
...
# STREAMLIT UI
# -------------------------
st.set_page_config(page_title="SMO AI Chat", page_icon="💬", layout="centered")

st.title("💬 SMO AI Chat")
st.markdown("Ask SMO AI questions about marketing and Redshift data insights. Powered by Amazon Bedrock.")
...
for msg in st.session_state.chat_history:
    if msg["role"] == "user":
        ...
        st.chat_message("assistant").write(msg["content"])
        if msg.get("corresponding_query"):
            with st.expander("🛢️ Corresponding SQL Query"):
                st.code(msg["corresponding_query"], language="sql")
